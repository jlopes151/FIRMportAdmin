...
function FilteredPagedGrid(args) {
    ...
    var _init = function(){
        ...
        this.rebuildTheGrid = function(){
            ...
            */
            bfdsmgr.firm.setSearchName($('#srchIn').val());
            
            // destroy the existing datagrid before the new init
            _destroy();

            bfdsmgr.firm.initializeFilteredGrid();
    ...
    var _buildFilterGrid = function(){
        ...
        $("<span style='position: relative; left: 0px;'>Long Name:</span>").appendTo($(_gridDiv)).get(0);
        $("<input id='srchIn' style='position: relative; left: 10px;' size='25' maxlength='25' />").appendTo($(_gridDiv)).get(0);
        $("<input id='srchbtn' style='position: relative; left: 20px;' value='Submit' type='button' onclick='rebuildTheGrid();'/>").appendTo($(_gridDiv)).get(0);
