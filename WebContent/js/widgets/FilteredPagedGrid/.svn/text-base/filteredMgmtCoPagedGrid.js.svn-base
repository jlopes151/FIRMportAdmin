...
function FilteredPagedGrid(args) {
    ...
    var _init = function(){
        ...
        this.rebuildTheGrid = function(){
            ...
            */
            bfdsmgr.mgmtco.setSearchInput( document.getElementById('company').value, document.getElementById('system').value );

            // destroy the existing datagrid before the new init
//            $('.pagedGridBorder').remove();
            _destroy();

            bfdsmgr.mgmtco.initializeFilteredGrid();

            // delay the building of the grid until dates are entered
            _buildPagedGrid();
        };
    };
    ...
    var _buildFilterGrid = function(){
        ...
        $("<span style='position: relative; left: 0px;'>Company:</span>").appendTo($(_gridDiv)).get(0);
        $("<input id='company' style='position: relative; left: 10px; 'size='2' maxlength='2' />").appendTo($(_gridDiv)).get(0);
        $("<span style='position: relative; left: 20px;'>System:</span>").appendTo($(_gridDiv)).get(0);
        $("<input id='system' style='position: relative; left: 30px; 'size='3' maxlength='3' />").appendTo($(_gridDiv)).get(0);
        $("<input id='srchbtn' style='position: relative; left: 40px;' value='Submit' type='button' onclick='rebuildTheGrid();'/>").appendTo($(_gridDiv)).get(0);
