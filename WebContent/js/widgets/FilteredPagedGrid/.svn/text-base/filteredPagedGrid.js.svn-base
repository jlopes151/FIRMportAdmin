...
function FilteredPagedGrid(args) {
    ...
    var _buildFilterGrid = function(){
        ...
        _gridDiv = $('<div class=pagedGridBorder width="'+_width+'">').attr("id", _id).addClass(_css).appendTo($(_attachPoint)).get(0);
         // append the filter label
        //$("<span>" + _filterLabel + ":&nbsp</span>").appendTo($(_gridDiv)).get(0);
        // append the filter input box
        //$("<input type='text' id='filterInput' name='filterInput' />").appendTo($(_gridDiv)).get(0);
        // on click method should be set in appropriate JS file using JQUERY
        //$("&nbsp;&nbsp;<input id='clearFilter' type='button' value='Clear' />").appendTo($(_gridDiv)).get(0);
        $("</div>").appendTo($(_gridDiv)).get(0);
        //$("<div><br></div>").appendTo($(_gridDiv)).get(0);
